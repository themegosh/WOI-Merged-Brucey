...
void ObjectMgr::LoadGuildPhaseMasks()
{
    // For reload case
    mGuildPhases.clear();
    ...
    QueryResult result = CharacterDatabase.Query("SELECT `guid`, `phasemask` FROM guild_house_masks");
    ...
    if (!result)
    {
        ...
        sLog.outString();
        sLog.outErrorDb(">> Loaded 0 Guild Phase Mask definitions. DB table `guild_house_masks` is empty.");
        return;
    }
    ...
    do
    {
        ...
        GuildPhases guildPhases;
        guildPhases.phasemask			= fields[1].GetUInt64();

        mGuildPhases[guild_id] = guildPhases;

        ++count;
    } while (result->NextRow());

    sLog.outString();
    sLog.outString(">> Loaded %u Guild Phase Mask definitions", count);
}
