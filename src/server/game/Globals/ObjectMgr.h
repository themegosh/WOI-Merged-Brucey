...
class ObjectMgr
{
    ...
    public:
		...
		typedef UNORDERED_MAP<uint64, GuildPhases > GuildPhasesMap;
		...
		GuildPhases const* GetGuildPhase(uint32 guildid) const
        {
            GuildPhasesMap::const_iterator itr = mGuildPhases.find(guildid);
            if (itr != mGuildPhases.end())
                return &itr->second;
            return NULL;
        }
